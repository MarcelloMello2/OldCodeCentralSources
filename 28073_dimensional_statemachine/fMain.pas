...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, uDimensionalStateMachine;
...
type
  ...
  protected
    ...
    procedure DoException(E: Exception; State: integer; const Dimension: string; const Data: TCalcData; var Handled: boolean); override;
  public
    stateDigit,
    stateDot,
    stateAdd,
  ...
  private
    FStateMachine: TCalcStateMachine;
    procedure AddTrace(const State: string; const Dimension: string; Valid: boolean; const Data: TCalcData);
  end;
...
procedure TfmCalculator.AddTrace(const State, Dimension: string; Valid: boolean; const Data: TCalcData);
begin
  memTrace.Lines.Add(Format('State: %s Valid: %s Dimension: %s Output: %s',[State,BoolToStr(Valid,True),Dimension,Data.Output]));
end;

procedure TfmCalculator.btnClick(Sender: TObject);
begin
  FStateMachine.Data.Argument := TButton(Sender).Caption[1];
  FStateMachine.Run(TButton(Sender).Tag);
  edOutput.Text := FStateMachine.Data.Output;
end;

procedure TfmCalculator.FormCreate(Sender: TObject);
begin
  FStateMachine := TCalcStateMachine.Create;
  btnDot.Caption :=  FormatSettings.DecimalSeparator;
  FStateMachine.OnLog := AddTrace;
end;

procedure TfmCalculator.FormDestroy(Sender: TObject);
begin
  FStateMachine.Free;
end;
...
procedure TCalcStateMachine.DoException(E: Exception; State: integer; const Dimension: string; const Data: TCalcData; var Handled: boolean);
var
  OddMessage: boolean;
begin
  ...
  inherited DoException(E,State,Dimension,Data,Handled);
end;
...
begin
  Data.Input := FloatToStr(Data.Value * (Data.InputValue/100.0));
  Data.SetCompute(nil);
  Data.Output := FloatToStr(Data.Value);
end;
