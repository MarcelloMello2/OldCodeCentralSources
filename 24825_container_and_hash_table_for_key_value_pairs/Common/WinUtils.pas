...
{ ! <summary>
  ...
  <para>Last modified       2009-02-09</para>
  </history>
  <remarks></remarks> }
{ ====================================================================== }
...
function FindDDECommands(const doctype, command: string; var serverapp, topic,
  service, ddecommand: string): Boolean;

function FindServerApp(const docextension: string): string;
function IsDesignerInstance(hw: HWND): Boolean;
{$IFNDEF COMPILER2_UP}
function WinExecAndWait(const Cmd: string; cmdShow: Cardinal): Cardinal;
{$ELSE}
...
uses SysUtils, Clipbrd,
  Registry, Utils, ShellAPI, ActiveX, Charsets, UnicodeHelpersU;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if p <> nil then
          try
            S.Write(p^, GlobalSize(handle)) finally GlobalUnlock(handle);
          end;
      end; { If }
    finally
      Close
...
const
{$IFDEF WIN32}
  ServerKeys: array [0 .. 3] of string = ('LocalServer32', 'LocalServer',
    'InProcServer32', 'InProcServer');
{$ELSE}
  ServerKeys: array [0 .. 3] of string = ('LocalServer', 'LocalServer32',
...
{$IFDEF WIN32}
      Access := KEY_READ;
{$ENDIF}
      if OpenKey(OleObjName + '\CLSID', False) then
      begin
        temp := readString(EmptyStr);
        CloseKey;
        if OpenKey('CLSID\' + temp, False) then
        begin
          for i := 0 to High(ServerKeys) do
            if KeyExists(ServerKeys[i]) then
            begin
...
{$IFDEF WIN32}
        Access := KEY_READ;
{$ENDIF}
        if OpenKey(doctype, False) then
        begin
          temp := readString(EmptyStr);
          CloseKey;
          if OpenKey(temp + '\shell\' + command, False) then
          begin
            ...
            Result := (Length(serverapp) > 0) and (Length(ddecommand) > 0) and
              (Length(topic) > 0) and (Length(service) > 0);
            CloseKey;
          end; { If }
        end; { If }
      end; { With }
...
{$IFDEF WIN32}
        Access := KEY_READ;
{$ENDIF}
        if OpenKey(docextension, False) then
        begin
          temp := readString(EmptyStr);
          CloseKey;
          if OpenKey(temp + '\shell\open\command', False) then
          begin
            temp := readString(EmptyStr);
            i := Scan('%', temp);
            if i > 0 then
...
{$IFDEF COMPILER2_UP}
    ...
    Result := id1 = id2;
{$ELSE}
    Result := GetWindowTask(hw) = GetWindowTask(designer);
{$ENDIF}
  end; { If }
...
begin
  ...
  try
    ...
    if hMod < 32 then
      ...
      while GetModuleUsage(hMod) > 0 do
      begin
