...
<para>Version 1.1 created 2008-10-05, modified for Delphi 2009</para>
<para>Version 1.2 created 2009-04-01, added Contains method.</para>
<para>Last modified       2009-04-03</para>
</history>
<remarks>  <para>
...
<para>
 === CAVEAT! ===</para>
<para>
 The node store uses the first sizeof(pointer) bytes to chain free
 nodes into a linked list. If you return a node to the store these bytes
 will be overwritten after the cleanup proc has been called!
 <b>Do not store compiler-managed types like
 strings, interfaces, or dynamic arrays into the first field of a node
 record!</b> A cleanup proc called when the store is
 destroyed may interpret the pointer link as an interface or other managed
 type and try to properly free it. This blows up spectacularly, resulting
 in very hard to trace exceptions (usually external exceptions). Been
 there, done that, have the scars to show for it... </para>
<para>
 This problem should be fixed now, the TBlockStore.PerformCleanup
 method now nils the link before it calls the cleanup proc.</para> </remarks>}
{======================================================================}
...
type
  ...
  public
    ...
    {! Checks if the passed pointer is for a node in this store. }
    function Contains(aNode: Pointer): boolean;
...
begin
  ...
  for i:= 1 to FBlocksize do begin
    PPointer(walker)^ := nil;  // attempt to prevent the blowup if first
                               // node field is a compiler managed type
                               // aCleanupProc may try to free.
    aCleanupProc( walker );
    {$IFDEF UNICODE}
...
function TPBNodeStore.Contains(aNode: Pointer): boolean;
begin
  Result := IsValidNodePtr(aNode);
end;
