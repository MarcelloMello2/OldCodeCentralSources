...
type
  {!
   <remarks>
    Callback prototype used for IEnumHashtable.EnumEntries. </remarks>
   <returns>
    true to continue the enumeration, false to stop it. </returns>
   <param name="aIntf">
    represents the hash item in the enumerated slot.</param>
  }
  TEnumCallback = function(aIntf: IHashable): Boolean of Object;
  {!
   <remarks>
    The hash table implemented by this unit supports this interface to
    enumerate all entries in the hashtable.</remarks>
  }
  IEnumHashtable = interface
  ['{AD91CE69-F591-4FB6-AFD9-F02D527F666D}']
    {!
    <summary>
     Enumerate all entries in the hashtable.</summary>
    <returns>
     the interface for the entry for which the callback returned
     false, or nil, if it always returned true.</returns>
    <param name="aCallback">
     is the method to pass each entry to. Enumeration will continue
     until the callback returns false or all entries have been enumerated.
     This parameter cannot be nil.</param>
    <exception cref="EParameterCannotBeNil">
     is raised if acallback is nil.</exception>
    }
    function EnumEntries(aCallback: TEnumCallback): IHashable;
  end;
...
uses
  Classes, Sysutils, Math, PBNodeStoreU, PrimesU, Windows, CommonTypesU;
...
function TPBInterfaceHashtable.EnumEntries(aCallback: TEnumCallback):
    IHashable;

  function EnumChain(walker: PIntfListNode): IHashable;
  begin
    Result := nil;
    while Assigned(walker) and not Assigned(Result) do begin
      if not aCallback(walker.Intf) then
        Result := walker.Intf;
      walker := walker^.Next;
    end;
  end;
...
begin
  if not Assigned(aCallback) then
    raise EParameterCannotBeNil.Create('TPBInterfaceHashtable.EnumEntries','aCallback');

  Result := nil;
  for i:= 0 to High(Slots) do begin
    Result := EnumChain(Slots[i]);
