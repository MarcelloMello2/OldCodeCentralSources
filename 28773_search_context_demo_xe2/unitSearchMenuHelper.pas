...
uses
  FMX.Effects, FMX.Layouts, FMX.Objects, System.Classes, System.UITypes,
  FMX.Types, FMX.Controls, Generics.Collections, Math, System.SysUtils;
...
procedure Register;
begin
  RegisterComponents( 'Samples', [ TSearchBandManager ] );
end;
...
begin
  ...
  BackGroundRec.Align := TAlignLayout.alClient;
  BackGroundRec.Fill.Color := BackGroundColor;
  BackGroundRec.Fill.Kind := TBrushKind.bkSolid;
  ...
  BackGroundGlow.Stroke.Kind := TBrushKind.bkSolid;
  BackGroundGlow.Opacity := 0.85;
  BackGroundGlow.Fill.Color := BackGroundColor;
  BackGroundGlow.Fill.Kind := TBrushKind.bkSolid;
  BackGroundGlow.Align := TAlignLayout.alClient;
  ...
  Image.Align := TAlignLayout.alCenter;
  Image.HitTest := False;

  Stream := TMemoryStream.Create;
  try
    SourceImage.SaveToStream(Stream);
    Stream.Position := 0;
  ...
  FooterLabel.Align := TAlignLayout.alBottom;
  FooterLabel.TextAlign := TTextAlign.taCenter;
...
begin
  ...
  Self.FItemGrid.Align := TAlignLayout.alClient;
...
begin
  ...
  for Band in Self.FSearchBands do
    Band.Align := TAlignLayout.alMostTop;

  Self.FSearchBands.Add(SearchBand);
  SearchBand.Parent := Self;
  SearchBand.Align := TAlignLayout.alTop;
